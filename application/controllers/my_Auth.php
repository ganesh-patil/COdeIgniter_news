...
class Auth extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->add_package_path(APPPATH.'third_party/ion_auth', FALSE);
		$this->load->database();
	...
	function register()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			$activation_code = random_string('alnum', 25);
			$additional_data = array(
                'activation_code' => $activation_code,
            );
			if ($this->form_validation->run() == true && $this->ion_auth->register($email, 'Ganesh@123', $email,$additional_data))
			{
				$this->send_email($email,$activation_code);
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("auth", 'refresh');
			}
		}
...
//            );
			$old_data = $this->user->get_identity_column_value($identity_column,$id);
//			print_r($old_data );die;
			$change = $this->ion_auth->change_password($old_data->$identity_column, $old_data->password, $password);
			var_dump($change);die;
        }

        if ($this->form_validation->run() == true && $this->user->update_user_password($id, $password))
        {
...
//            );
	...
	function _render_page($view, $data=null, $returnhtml=false)//I think this makes more sense
	{
		...
		$this->viewdata['partial'] = $view;
		$view_html = $this->load_view($this->viewdata, $returnhtml);
